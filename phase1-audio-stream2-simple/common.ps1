...
$script:CandidateName = "pocketjs-a1099-phase1-audio-stream2.ipod"
$script:CandidateSha256 = "c3f0b9a549707ceeeb791e1f3e2be815ab0ab02e47db25f96e2a2713ac1dcabd"
$script:TransactionName = ".pocketjs-audio-stream2-state.json"
$script:BackupName = ".pocketjs-audio-stream2-backup"
$script:ManagedFiles = @(
...
function Get-LineageState {
    ...
    if ($state.schema -ne 1 -or $state.transaction -ne "phase1-audio-stream2") {
...
function Start-LineageTransaction {
    ...
    try {
        ...
        $state = [ordered]@{
            schema = 1
            transaction = "phase1-audio-stream2"
            phase = "snapshot"

...
Assert-ExpectedHash -Path (Join-Path $PSScriptRoot "APP.PKT") -Expected $script:AppSha256 | Out-Null
Assert-ExpectedHash -Path (Join-Path $PSScriptRoot "STATE0.BIN") -Expected $script:State0Sha256 | Out-Null
Assert-ExpectedHash -Path (Join-Path $PSScriptRoot "STATE1.BIN") -Expected $script:State1Sha256 | Out-Null

try {
    Write-Host "Backing up APP.PKT and both state slots..."
    Start-PersistenceTransaction -Mount $root | Out-Null
    ...
    try { Restore-PersistenceTransaction -Mount $root } catch {
        Write-Warning "Automatic persistence rollback failed: $($_.Exception.Message)"
    }
    throw
}

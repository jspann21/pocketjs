...
static void make_disk(void) {
    ...
    put32(b+32,70000);
    put32(p+12,70000);
    uint8_t *fat=disk[2]; put32(fat+0,0x0ffffff8); put32(fat+4,0xffffffff); put32(fat+8,0x0fffffff); put32(fat+12,0x0fffffff); put32(fat+16,0x0fffffff);
    const char dname[11]={'P','O','C','K','E','T','J','S',' ',' ',' '};
    const char fname[11]={'A','P','P',' ',' ',' ',' ',' ','P','K','T'};
    dirent(disk[549],dname,0x10,3,0);
    dirent(disk[550],fname,0x20,4,7);
    memcpy(disk[551],"POCKET!",7);
}
int main(void) {
    make_disk();
    PjsFat32 fat;
    assert(pjs_fat32_mount(&fat,read_sector,0)==PJS_STORAGE_OK);
    uint8_t out[32]={0}; uint32_t length=0;
    const char dname[11]={'P','O','C','K','E','T','J','S',' ',' ',' '};
    const char fname[11]={'A','P','P',' ',' ',' ',' ',' ','P','K','T'};
    uint32_t size=0;
    assert(pjs_fat32_short_file_size(&fat,dname,fname,&size)==PJS_STORAGE_OK && size==7);
    assert(pjs_fat32_read_short_file(&fat,dname,fname,out,sizeof(out),&length)==PJS_STORAGE_OK);
    assert(length==7 && memcmp(out,"POCKET!",7)==0);
    puts("fat32 read-only loader: OK");
    return 0;
}

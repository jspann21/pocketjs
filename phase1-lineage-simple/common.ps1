...
$script:CandidateName = "pocketjs-a1099-phase1-lineage.ipod"
$script:CandidateSha256 = "9fce741604fe5d7b79506d8de73cf9794a54ae8b1f540d552031b1a21f991e71"
$script:TransactionName = ".pocketjs-lineage-state.json"
$script:BackupName = ".pocketjs-lineage-backup"
$script:ManagedFiles = @(
...
$script:StagedFiles = [ordered]@{
    ...
    "PENDING.PKT" = "b58d7cbeae0545cfaa5e0de3c9028ca18d98d3b4dd841f1f5934a0f3e298a58c"
...
function Get-LineageState {
    ...
    if ($state.schema -ne 1 -or $state.transaction -ne "phase1-lineage") {
...
function Restore-LineageTransaction {
    ...
    foreach ($item in @($state.files)) {
        ...
        if ($script:StagedFiles.Contains([string]$item.name) -and -not $isState -and
            ...
            if ($item.existed) {
                Assert-ExpectedHash -Path $current -Expected ([string]$item.sha256) | Out-Null
            } elseif (Test-Path -LiteralPath $current) {
                throw "Unexpected file appeared during the gate: $current"
            }
        }
        if ($item.existed) {

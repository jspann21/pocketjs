...
if (Test-Path -LiteralPath $paths.State -PathType Leaf) {
    $packageState = Get-MultiState -Paths $paths
    if ($packageState.phase -ne "staged") {
        throw "Multi-app state says '$($packageState.phase)', expected 'staged'."
    }
...
if ((Test-Path -LiteralPath $paths.State) -or (Test-Path -LiteralPath $paths.Backup)) {
    throw "Multi-app restore verification failed. Do not disconnect the iPod."
}
...
$result = [ordered]@{
    ...
    multiAppTransactionExists = $false
}

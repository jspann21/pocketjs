...
if ($before.backupExists -or $null -ne $before.state) {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "handoff restore failed with exit code $LASTEXITCODE"
    }
} else {
...
if (-not $after.targetExists -or $after.backupExists -or $null -ne $after.state) {
    throw "Restore verification failed. Do not disconnect the iPod."
}
...
Write-Host "Safely eject the iPod and confirm that Rockbox boots normally."
